--[[
    Delta UI v2 Rebirth
    Recriado com fidelidade total às imagens fornecidas.
    Funcionalidades: Salvamento em workspace, Execução real, Minimização, Logs e Busca.
]]
...
-- Proteção contra detecção simples (dependendo do executor)
...
-- Garantir que a pasta existe (apenas se o executor suportar)
if makefolder then
    pcall(function()
        makefolder("Deltav2")
...
MinimizedOrb.Image = "rbxassetid://15286283582" -- Ícone do Delta
MinimizedOrb.Parent = ScreenGui
local OrbCorner = Instance.new("UICorner")
OrbCorner.CornerRadius = UDim.new(1, 0)
OrbCorner.Parent = MinimizedOrb
...
-- Bolinhas Decorativas (Roxo, Vermelho, Amarelo)
...
local function createTabIcon(id, pos, name)
    ...
    btn.MouseButton1Click:Connect(function()
        currentTab = name
        -- Atualizar visual das abas e visibilidade do container
        -- (Lógica de troca implementada abaixo)
    end)
    return btn
end
...
-- Seleção visual (o quadrado branco em volta do ícone)
...
ScriptBox.Size = UDim2.new(0.65, -10, 0.8, -10)
...
-- Sidebar do Editor (Network e Infinite Yield)
local Sidebar = Instance.new("Frame")
Sidebar.Size = UDim2.new(0.35, -10, 1, -10)
Sidebar.Position = UDim2.new(0.65, 5, 0, 5)
...
local NCC = Instance.new("UICorner")
NCC.CornerRadius = UDim.new(0, 8)
NCC.Parent = NetCard
...
local IYEC = Instance.new("UICorner")
IYEC.CornerRadius = UDim.new(0, 12)
IYEC.Parent = IYExec
local IYS = Instance.new("UIStroke")
...
-- Botões de Baixo: Execute e Save
local ExecuteBtn = Instance.new("TextButton")
ExecuteBtn.Size = UDim2.new(0, 150, 0, 45)
ExecuteBtn.Position = UDim2.new(0, 10, 1, 10)
...
ExecuteBtn.MouseButton1Click:Connect(function()
    ...
    if not success then
        warn("Delta v2 Error: " .. err)
    end
end)

-- Lógica de Salvar no Workspace
...
-- Loop de Update (FPS e Ping)
RunService.RenderStepped:Connect(function(dt)
    if EditorPage.Visible then
        -- Calcula o FPS baseando-se no tempo entre frames (DeltaTime)
        local fps = math.floor(1/dt)
        -- Puxa o Ping real do servidor através dos Stats do jogo
        local ping = math.floor(game:GetService("Stats").Network.ServerStatsItem["Data Ping"]:GetValue())
        -- Conta quantos jogadores estão no servidor atual
        local players = #game:GetService("Players"):GetPlayers()
        -- Atualiza o texto na sidebar do Editor conforme a primeira print
        NetInfo.Text = string.format("Client status information\n%d FPS\n%d MS Ping\n%d Players", fps, ping, players)
    end
end)

-- Mensagem de carregamento final para confirmar que tudo subiu sem erros
print("Delta v2 Loaded Successfully!")
